...
- **Import** by choosing a `.yaml` file, dropping one onto the page, or pasting YAML (1 MB limit) on
  the single **Import** page. A chosen or dropped file is read into the text box, so you can see and
  edit it before importing. The format is detected
